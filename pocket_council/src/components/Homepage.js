        ...
        <div className="min-h-screen bg-[#0f172a] text-white">
            ...
            <main className="px-4 py-6 max-w-md mx-auto space-y-6">
                ...
                <div>
                    ...
                    {showPatientList && (
                        <div className="mt-3 bg-linear-to-b from-slate-800/90 to-slate-900/90 border-2 border-blue-500/30 rounded-2xl overflow-hidden backdrop-blur-md shadow-2xl shadow-blue-500/20 animate-in fade-in slide-in-from-top duration-300">
                            {PATIENTS.map((patient, index) => (
                                <button
                                    ...
                                    className="w-full p-4 flex items-center gap-4 hover:bg-linear-to-r hover:from-blue-500/20 hover:to-violet-500/20 transition-all duration-300 border-b border-blue-500/10 last:border-b-0 group relative overflow-hidden"
                                    style={{ animationDelay: `${index * 50}ms` }}
                                >
                                    {/* Hover effect background */}
                                    <div className="absolute inset-0 bg-linear-to-r from-blue-500/0 via-blue-500/5 to-violet-500/0 opacity-0 group-hover:opacity-100 transition-opacity duration-300"></div>
                                    
                                    <div className="w-12 h-12 rounded-xl bg-linear-to-br from-blue-500 to-violet-500 flex items-center justify-center shadow-lg group-hover:scale-110 transition-transform duration-300 relative z-10">
                                        <span className="text-xl">👤</span>
                                        {/* Active indicator for selected patient */}
                                    ...
                                    <div className="text-left flex-1 relative z-10">
                                        <div className="font-bold text-white text-base group-hover:text-blue-300 transition-colors">{patient.name}</div>
                                        <div className="text-xs text-slate-400 group-hover:text-slate-300 transition-colors font-medium">{patient.age}y • {patient.condition}</div>
                                    </div>
                                    {/* Right arrow indicator */}
                                    <ChevronDown className="w-5 h-5 text-blue-400 -rotate-90 opacity-0 group-hover:opacity-100 transform translate-x-[-10px] group-hover:translate-x-0 transition-all duration-300 relative z-10" />
                                </button>
                            ))}
                        </div>
                    )}
                ...
                {/* Patient Info Card */}
                <div className="relative bg-linear-to-br from-slate-800/80 to-slate-900/80 border-2 border-slate-700/50 rounded-2xl p-5 backdrop-blur-sm shadow-xl overflow-hidden group">
                    {/* Animated background glow */}
                    <div className="absolute inset-0 bg-linear-to-r from-blue-500/5 via-violet-500/5 to-blue-500/5 opacity-0 group-hover:opacity-100 transition-opacity duration-500"></div>
                    
                    {/* Header */}
                    <div className="flex items-center gap-2 mb-4 relative z-10">
                        <div className="w-8 h-8 rounded-lg bg-linear-to-br from-blue-500/20 to-violet-500/20 flex items-center justify-center border border-blue-500/30">
                            <Stethoscope className="w-4 h-4 text-blue-400" />
                        </div>
                        <h3 className="text-xs font-bold text-blue-300 uppercase tracking-wider">Medical Information</h3>
                    </div>
                    
                    <div className="grid grid-cols-2 gap-4 relative z-10">
                        <div className="bg-white/5 rounded-xl p-3 border border-red-500/20 hover:border-red-500/40 transition-colors group/item">
                            <div className="flex items-center gap-2 mb-2">
                                <AlertTriangle className="w-3.5 h-3.5 text-red-400" />
                                <div className="text-[10px] text-red-400 font-semibold uppercase tracking-wide">Allergies</div>
                            </div>
                            <div className="text-sm font-bold text-white group-hover/item:text-red-200 transition-colors">{selectedPatient.allergies}</div>
                        </div>
                        <div className="bg-white/5 rounded-xl p-3 border border-emerald-500/20 hover:border-emerald-500/40 transition-colors group/item">
                            <div className="flex items-center gap-2 mb-2">
                                <FileText className="w-3.5 h-3.5 text-emerald-400" />
                                <div className="text-[10px] text-emerald-400 font-semibold uppercase tracking-wide">Medications</div>
                            </div>
                            <div className="text-sm font-bold text-white group-hover/item:text-emerald-200 transition-colors">{selectedPatient.meds}</div>
                        </div>
                    </div>
                </div>
